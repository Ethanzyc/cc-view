# 等输入闲置降级 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 等输入会话晾置超 30 分钟时，在 overlay「待介入」section 内降级——project 内排末尾 + 灰显 + 「闲置」标签；整组全闲置的 project 沉到 section 底部。纯前端 derived，不动后端。

**Architecture:** 前端纯函数 `isStaleInput(s, now)` 判定 → 响应式 `now` ref（60s tick）驱动 computed / 模板定期重算 → `sorted` 加闲置键 → `groups` active 段 project 重排 → 行 `idle` class + 标签。后端 emit 有 hash 去重（不追踪时间），故前端自己 tick 保证跨阈值时自动触发。

> **修正记录**：实现过程中目标从等回答（waitingForReply）修正为等输入（waitingForInput），标签「超时」→「闲置」，class `stale`→`idle`。下方 Task 历史步骤经批量替换，代码示例可能有少量残留（如 reply class 的状态值），**以实际代码为准**。
...
- **注释中文，代码英文**（遵循用户全局规范）。
- **时间响应（关键）**：后端 `lib.rs hash_sessions` 只按 `(id, status, alive, snoozed)` hash，数据不变不 emit。`isStaleInput` 依赖时间，必须前端加 `now` ref + `setInterval(60s)` 定期触发重算，否则晾着的等输入跨 30min 阈值时不会自动变闲置。
- **手动验证技巧**：dev 模式下临时把 `STALE_INPUT_MS` 改成 `3_000`（3 秒），打开 overlay 观察一个 `等输入` 会话在几秒后变灰 + 排后；验证后改回 `30 * 60 * 1000`。
...
- `src/utils/session.ts`（改）—— 加 `STALE_INPUT_MS` 常量 + `isStaleInput(s, now)` 纯函数。
- `src/components/Overlay.vue`（改）—— `now` ref + 60s 定时器；`sorted` computed 加闲置键；`groups` active 段 project 重排；搜索态 + 非搜索态两个 row 模板加 `idle` class + 「闲置」标签；`.row.idle` / `.idle-tag` 样式。

---

### Task 1: isStaleInput(s, now) 判定函数

**Files:**
- Modify: `src/utils/session.ts`（在 `isFresh` 函数附近）

**Interfaces:**
- Produces: `export const STALE_INPUT_MS: number`（`30 * 60 * 1000`）；`export function isStaleInput(s: Session, now: number): boolean`
...
```ts
// 等输入闲置阈值：超过此时长未处理的等输入在 overlay 降级（灰显 + 排后）。
export const STALE_INPUT_MS = 30 * 60 * 1000;

// 等输入晾置闲置：waitingForInput + 非搁置 + 距 statusUpdatedAt 超阈值。
// now 由调用方传入（响应式 now ref），让前端能定期重算——后端 emit 有 hash 去重，
// 不传时间晾着的等输入跨阈值时不会自动触发。仅对等输入（等输入是正常间隔，等权限太重要）。
...
git commit -m "feat: 加 isStaleInput 判定（等输入超 30min 降级依据）"
```

---

### Task 2: now ref + 定时器 + sorted 闲置键

**Files:**
- Modify: `src/components/Overlay.vue`（顶部 import + setup 区 ref + `onMounted` + `sorted` computed）

**Interfaces:**
- Consumes: `isStaleInput` from `../utils/session`（Task 1）
...
```
改为（读 `now.value` 建立响应式依赖 + 第一键 isStaleInput）：
```ts
const sorted = computed(() => {
...
dev 模式，临时 `STALE_INPUT_MS = 3_000`。找一个等输入会话，观察 ~60s 内（now tick）它会排到 project 组末尾（此时还没灰显/标签，那是 Task 4；全闲置 project 下沉是 Task 3）。验证后改回 `30 * 60 * 1000`。

- [ ] **Step 6: Commit**

```bash
git add src/components/Overlay.vue
git commit -m "feat: sorted 加闲置等输入排末尾 + now tick 定时刷新"
```

---

### Task 3: 待介入组全闲置 project 整组下沉

**Files:**
- Modify: `src/components/Overlay.vue`（`groups` computed 的 active 段）

**Interfaces:**
- Consumes: `isStaleInput`（Task 1）；`now` ref（Task 2）；`byProj`（同 computed 内已有）
...
dev 模式，临时 `STALE_INPUT_MS = 3_000`。造一个 project 下全是等输入会话（都晾超阈值），观察该整组沉到「待介入」section 最底；含任一非闲置项的 project 保持原位。验证后改回 `30 * 60 * 1000`。

- [ ] **Step 4: Commit**

```bash
git add src/components/Overlay.vue
git commit -m "feat: 待介入组全闲置 project 整组下沉"
```

---

### Task 4: 行 stale 视觉（灰显 + 闲置标签）

**Files:**
- Modify: `src/components/Overlay.vue`（搜索态 + 非搜索态两个 row 模板的 `:class` 与 `ago` 区；`<style>` 加 `.row.idle` / `.idle-tag`）

**Interfaces:**
- Consumes: `isStaleInput`（Task 1）；`now` ref（Task 2，模板自动 unwrap）

- [ ] **Step 1: 非搜索态 row 加 stale class**

非搜索态 row 的 `:class`（约 line 348-354），加 `idle: isStaleInput(s, now),`：
```ts
            :class="{
              dead: !s.alive,
              snoozed: s.snoozed,
              perm: s.status === 'needsPermission' && !s.snoozed,
              reply: s.status === 'waitingForInput' && !s.snoozed,
...
搜索态 row 的 `:class`（约 line 268-273），同样加 `idle: isStaleInput(s, now),`：
```ts
          :class="{
            dead: !s.alive,
            snoozed: s.snoozed,
...
dev 模式，临时 `STALE_INPUT_MS = 3_000`：
1. 一个等输入会话晾超阈值（~60s 内 now tick）→ 行 opacity 0.5 灰显 + ago 旁出现「闲置」标签 + 排到 project 末尾。
2. 把该会话手动隐藏（点「隐藏」）→ opacity 变 0.35（is-hidden 覆盖 stale），「闲置」+「已隐藏」标签都在。
3. 该会话状态变 working（在 cc 里回答它，触发 emit）→ 立即取消灰显 + 归位。
4. 验证后改回 `STALE_INPUT_MS = 30 * 60 * 1000`。

- [ ] **Step 7: Commit**

```bash
git add src/components/Overlay.vue
git commit -m "feat: 闲置等输入灰显 + 闲置标签"
```
...
**Spec coverage：**
- 判定（isStaleInput + 30min + 非snoozed + 仅等输入）→ Task 1 ✓
- 时间自动刷新（跨阈值触发）→ Task 2 now ref + 60s tick ✓（修正了 hash 去重 emit 不追踪时间的 gap）
- sorted project 内排末尾 → Task 2 ✓
- 全闲置 project 整组沉底 → Task 3 ✓
- 灰显 opacity 0.5 + 「闲置」标签 → Task 4 ✓
...
**Type consistency：** `isStaleInput(s: Session, now: number): boolean` 全程一致（Task 1 定义，Task 2/3/4 消费）；`now: Ref<number>` 在 Task 2 定义后被 sorted / groups / 模板一致使用。✓
